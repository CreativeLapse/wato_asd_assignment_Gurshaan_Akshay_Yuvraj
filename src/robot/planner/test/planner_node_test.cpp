...
class PlannerNodeTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    odom.pose.pose.position.x = -0.7;  // Wheel axle at -2.0, lidar 1.3 m ahead.
...
TEST_F(PlannerNodeTest, RetainsGoalAcrossTemporaryPlanningFailure) {
  ...
  ASSERT_FALSE(received.empty());
  ASSERT_FALSE(received.back().poses.empty());
  for (int y = 0; y < 20; ++y) { map.data[y * 20 + 10] = 100; }
  maps->publish(map);
  spin(0.2);
  ...
  validator.configure(50, 30, 3.0, 10);
...
TEST_F(PlannerNodeTest, ReplacesStillValidDetourWhenAShortcutOpens) {
  for (int y = 0; y < 15; ++y) { map.data[y * 20 + 10] = 100; }
  maps->publish(map);
  spin(0.2);
  sendGoal();
  ASSERT_FALSE(received.empty());
  ...
  // Soft inflation changes do not justify a longer route or an equal-cost
  // replacement. This also exercises repeated timer optimality checks.
