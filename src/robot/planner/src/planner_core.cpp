...
{
  if (path.poses.empty() || path.header.frame_id != map.header.frame_id ||
  ...
  // Paths generated by A* have adjacent cell centres. Also sample segments
  // so a changed grid resolution cannot hide an obstacle between waypoints.
  CellIndex previous;
  bool have_previous = false;
  for (size_t i = nearest; i < path.poses.size(); ++i) {
    const auto& end = path.poses[i].pose.position;
    const auto& begin = path.poses[i == nearest ? i : i - 1].pose.position;
    const int steps = std::max(1, static_cast<int>(std::ceil(
      std::hypot(end.x - begin.x, end.y - begin.y) / (0.25 * map.info.resolution))));
    for (int step = 0; step <= steps; ++step) {
      const double t = static_cast<double>(step) / steps;
      CellIndex cell;
      if (!worldToCell(begin.x + t * (end.x - begin.x),
                       begin.y + t * (end.y - begin.y), cell) || isBlocked(cell)) {
        return false;
      }
      if (have_previous && cell.x != previous.x && cell.y != previous.y &&
          (isBlocked(CellIndex(cell.x, previous.y)) ||
           isBlocked(CellIndex(previous.x, cell.y)))) {
        return false;
      }
      previous = cell;
      have_previous = true;
  ...
  const nav_msgs::msg::Path& path, double robot_x, double robot_y)
{
  ...
  map_ = map;
  CellIndex start;
  if (!worldToCell(robot_x, robot_y, start) ||
      (isBlocked(start) && !nearestOpenCell(start, start))) {
    return invalid;
  }

  // Compare routes from exactly the same grid cell as a fresh search.
  // A nearest-waypoint comparison can make a longer route appear cheaper
  // by silently skipping the distance from the new start to that waypoint.
  size_t begin = path.poses.size();
  for (size_t i = 0; i < path.poses.size(); ++i) {
    CellIndex cell;
    const auto& p = path.poses[i].pose.position;
    if (worldToCell(p.x, p.y, cell) && cell == start) {
      begin = i;
      break;
    }
  }
  if (begin == path.poses.size()) {
    return invalid;
  }
  double start_x, start_y;
  cellToWorld(start, start_x, start_y);
  if (!remainingPathIsValid(map, path, start_x, start_y)) {
    return invalid;
  }
  double cost = 0.0;
  for (size_t i = begin + 1; i < path.poses.size(); ++i) {
    const auto& a = path.poses[i - 1].pose.position;
    const auto& b = path.poses[i].pose.position;
    CellIndex cell;
    if (!worldToCell(b.x, b.y, cell)) {
      return invalid;
    }
    cost += std::hypot(b.x - a.x, b.y - a.y) / map.info.resolution *
      (1.0 + cost_weight_ * cellCost(cell) / 100.0);
  }
  return cost;
}
