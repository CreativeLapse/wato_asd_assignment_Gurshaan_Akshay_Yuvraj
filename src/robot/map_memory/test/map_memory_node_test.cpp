...
TEST(MapMemoryNodeTest, PlacesScanAtInterpolatedPoseAndUpdatesWhileStationary) {
  ...
  options.parameter_overrides({rclcpp::Parameter("global_map.inflation_radius", 0.0)});
  ...
  scans->publish(scan);  // Must not replace the scan waiting at t=11.
  spin(0.1);
  ...
  // At t=11 the sensor was x=1.0; using either latest or previous odom is wrong.
  EXPECT_EQ(maps.back().data[30 * 60 + 32], 100);
  EXPECT_EQ(maps.back().data[30 * 60 + 30], -1);
  // The robot then stands still. New free observations must still refresh.
  ...
  // A simulation clock reset must discard an old scan waiting in the queue.
